...
//double 실수형 더하는 과정에서 근사값문제로 BigDecimal을 사용해야 된다는 것을 알게됨
// BigDecimal을 처음 써보게 되는 문제였음 BigDecimal 어렵..
// 틀린 부분 수정 
// -1. 거리 계산 부분을 int형에서 long으로 변경
// -2. 이미 연결되어 있는 좌표 두 개의 번호가 오름차순으로 주어진다는 보장이 없어서 union 수정
// -3. 주요 수정 사항:
//BigDecimal 제거: BigDecimal을 제거하고, double을 사용해 계산 후 출력 시에 소수점 2자리까지 출력하도록 변경했습니다.
//M개의 연결된 통로 처리: 이미 연결된 통로는 union(tmpX, tmpY)로 처리하고 있습니다. 이 부분은 문제없지만, 중복된 연결이 발생하지 않도록 오름차순 처리 보장이 있는지 확인했습니다.
